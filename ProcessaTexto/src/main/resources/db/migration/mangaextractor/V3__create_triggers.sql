...
/*!50003 CREATE */ /*!50017 DEFINER = 'admin'@'%' */ /*!50003 TRIGGER tr_exemplo_paginas_insert BEFORE UPDATE ON exemplo_paginas FOR EACH ROW BEGIN    IF (NEW.id IS NULL OR NEW.id = '') THEN      SET new.id = UUID();    END IF;  END */$$

/*!50003 CREATE */ /*!50017 DEFINER = 'admin'@'%' */ /*!50003 TRIGGER tr_exemplo_paginas_update BEFORE UPDATE ON exemplo_paginas FOR EACH ROW BEGIN    SET new.Atualizacao = NOW();  END */$$
